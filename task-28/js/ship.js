...
function Ship(ship_id, orbit_id, energy_type, power_type){
	...
	//飞船广播系统执行
	setTimeout(sendBroadcastMessage, 1000);
	/**
	 * 飞船接收消息 -- 暴露对外的接口
	...
	function sendBroadcastMessage(){
		var obj = {};
		obj.ship_id = ship_id;
		obj.ship_status = ship_status==1?"move":"stop";
		obj.ship_fuel = Math.floor(ship_energy);
		var message = Universe.Adapter.encode(obj, "broadcast");
		Universe.Media.receive.broadcastMessage(message);
		Tool.logAdapter("info", "broadcast", message);
		if (ship_destroy == 1)
			setTimeout(sendBroadcastMessage, 1000);
		else 
			setTimeout(sendDestroyMessage, 1000);
	}
	function sendDestroyMessage(){
		var obj = {};
		obj.ship_id = ship_id;
		obj.ship_status = "destroy";
		obj.ship_fuel = Math.floor(ship_energy);
		var message = Universe.Adapter.encode(obj, "broadcast");
		Universe.Media.receive.broadcastMessage(message);
	}
