$(document).ready(function(){
	//生成轨道选择器
	$(".orbit-select-container").queen_select();
	$(".energy-select-container").queen_select();
	$(".power-select-container").queen_select();
	//创建飞船号
	var ship_id = 1;
	...
	$("div.control-board").on("click",".ship-add",function(event){
		...
		var energy_type =  $("div.energy-select-container select").attr("value");
		var power_type =  $("div.power-select-container select").attr("value");
		
		orbit_id = parseInt(orbit_id);
		energy_type = parseInt(energy_type);
		power_type = parseInt(power_type);
